    ...
    return (
      <div key={item["hero_id"]} className={herocontainerclass}>
        <div className="Hero-name">
          ...
          <div><img className="Hero-picture-teamid" src={heropicurl} alt={heroname + " picture"}></img> {heroname} </div> </div>
        <div className="Hero-games-played-teamid">
           <span className="Hero-info-label-teamid">Games played:</span> {gamesplayed}
         </div>
         <div className="Hero-wins-teamid">
           <span className="Hero-info-label-teamid">Wins:</span> {gameswon} ({Math.round(gameswon/gamesplayed*1000)/10}%)
          </div>
       </div>
    )
  })
